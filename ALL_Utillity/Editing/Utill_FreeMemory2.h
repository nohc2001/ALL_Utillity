...
struct HeapLumps {
	...
	void* AllocNewLump2MB() {
		...
		Lumps->push_back<Lump2MB*>(newlump);
		return newlump;
	}
	...
	void* AllocNewPages(ui32 size) {
		...
		if (pageArr->up + psiz > pageArr->capacity) {
			...
			for (int i = pageArr->up; i < pageArr->capacity; ++i) {
				SmallPage* sp = &((SmallPage*)((Lump2MB**)Lumps->Arr)[(i >> 9)]->data)[i & 511];
		...
		rptr = &((SmallPage*)((Lump2MB**)Lumps->Arr)[(pageArr->up >> 9)]->data)[pageArr->up & 511];
		pageArr->up += psiz;
	...
	si32 GetPageID(void* pageptr) {
		...
		for (int i = 0; i < Lumps->size(); ++i) {
			byte8* lumpptr = Lumps->at<byte8*>(i);
			si64 m = (reinterpret_cast<si64>(p) - reinterpret_cast<si64>(lumpptr)) / SMALL_PAGE_SIZE;
			if (0 <= m && m < 512) {
				return i * 512 + m;
			}
		}
...
public:
	...
	void FMSystemDump() {
		...
		for (int i = 0; i < fmhl.Lumps->up; ++i) {
			out << "Lump[" << i << "] : " << (int*)(((Lump2MB**)fmhl.Lumps->Arr)[0]) << "\t PageUp : ";
		...
		for (ui32 i = 0; i < fmhl.pageArr->up; ++i) {
			SmallPage* page = &((SmallPage*)((Lump2MB**)fmhl.Lumps->Arr)[(i >> 9)]->data)[i & 511];
...
public:
	...
	void push_back(T value)
	{
		...
		{
			Init(maxsize * 2, islocal, isdebug, fmlayer);
			Arr[up] = value;
			up += 1;
		}
	}
